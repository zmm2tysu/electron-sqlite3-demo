...
import { tables, updateSchema } from './migration'
...
async function setUsers(users: DB.UserInfo[]) {
  ...
  db.transaction((users) => {
    for (const user of users) {
      ...
      db.prepare(
        `
        INSERT OR REPLACE INTO ${tables.tt_users} (
          ${keys.join(',')}
        ) VALUES (
...
async function getUsers(onlyMcn?: boolean) {
  const db = getInstance()

  if (onlyMcn === void 0) {
    return prepare(db, `SELECT * FROM ${tables.tt_users} WHERE isDeleted = $isDeleted`).all({
      isDeleted: 0
    }) as DB.UserInfo[]
  } else {
    return prepare(
      db,
      `SELECT * FROM ${tables.tt_users} WHERE isDeleted = $isDeleted AND isMcn = $isMcn`
    ).all({ isDeleted: 0, isMcn: Number(onlyMcn) }) as DB.UserInfo[]
  }
}

async function removeUsersByMcnId(mcnId: string) {
  const db = getInstance()

  db.transaction((mcnId) => {
    //update isDeleted = 1
    prepare(db, `UPDATE ${tables.tt_users} SET isDeleted = 1 WHERE mcnId = $mcnId OR awemeNo = $mcnId`).run({ mcnId })
  })(mcnId)
}
