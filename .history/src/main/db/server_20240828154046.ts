...
async function getUsers(onlyMcn?: boolean) {
  ...
  if (onlyMcn === void 0) {
    return db
      .prepare(`SELECT * FROM ${tables.tt_users} WHERE isDeleted = $isDeleted`)
      .all({ isDeleted: 0 }) as DB.UserInfo[]
  } else {
    return db
      .prepare(`SELECT * FROM ${tables.tt_users} WHERE isDeleted = $isDeleted AND isMcn = $isMcn`)
...
async function removeUsersByMcnId(mcnId: string) {
  ...
  db.transaction((mcnId) => {
    db.prepare(`DELETE FROM ${tables.tt_users} WHERE mcnId = $mcnId`).run({ mcnId })
  })(mcnId)
}
