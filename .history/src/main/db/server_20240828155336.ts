...
async function getUsers(onlyMcn?: boolean) {
  ...
  if (onlyMcn === void 0) {
    return prepare(db, `SELECT * FROM ${tables.tt_users} WHERE isDeleted = 0`).all(
      {}
    ) as DB.UserInfo[]
  } else {
    return prepare(
      db,
      `SELECT * FROM ${tables.tt_users} WHERE isDeleted = 0 AND isMcn = $isMcn`
    ).all({ isMcn: Number(onlyMcn) }) as DB.UserInfo[]
  }
}
...
async function removeUsersByMcnId(mcnId: string) {
  ...
  db.transaction((mcnId) => {
    prepare(
      db,
      `UPDATE ${tables.tt_users} SET isDeleted = 1 WHERE mcnId = $mcnId OR awemeNo = $mcnId`
    ).run({ mcnId })
  })(mcnId)
}
