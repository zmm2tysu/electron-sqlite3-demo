...
export type DataInterface = {
  ...
  getUsers: (isMcn?: boolean) => Promise<DB.UserInfo[]>
  ...
  // // friends
  // setFriends: (owner: number, friends: DB.UserWithFriendSetting[]) => Promise<void>
  ...
  // updateFriendInfo: (owner: number, info: DB.FriendSetting & { id: number }) => Promise<void>
  ...
  // getMembersByGroupId: (owner: number, groupId: number) => Promise<DB.UserWithFriendSetting[]>
  ...
  // // messages
  // setMessage: (owner: number, message: ModuleIM.Core.MessageBasic) => Promise<void>
  // setMessages: (owner: number, messages: ModuleIM.Core.MessageBasic[]) => Promise<void>
  ...
  // // conversations
  // getLastConversationMessage: (owner: number, sender: number) => Promise<ModuleIM.Core.MessageBasic>
  ...
  // updateConversationLastRead: (id: string, lastReadAck: bigint) => Promise<void>
  ...
  // getConversations: (owner: number) => Promise<ModuleIM.Core.ConversationType[]>
  // getConversationsWithAll: (owner: number) => Promise<Array<ModuleIM.Core.ConversationWithAllType>>
  ...
  // //   }
  // // ) => number;
  // getTotalUnreadCount: (options: { owner: number; lastReadAck: bigint }) => Promise<number>
}
